...
router.post('/', requireRole('super_admin', ...BRANCH_ROLES), validate(CreateCustomerSchema), async (req: AuthRequest, res, next) => {
  try {
    ...
    const effectiveBranchId = isBranchRole(req.user!.role) ? req.user!.branchId! : branchId;
...
router.put('/:id', requireRole('super_admin', ...BRANCH_ROLES), validate(UpdateCustomerSchema), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (isBranchRole(req.user!.role) && existing.branch_id !== req.user!.branchId) {
      res.status(403).json({ error: 'Access denied' });
      return;
    }
